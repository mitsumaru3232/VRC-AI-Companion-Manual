...
会話の自然さ・文脈の理解・返答の正確さは、使用するAIサービスやモデルによって大きく異なります。同じサービスでもモデルによって差があります。
まず同じ内容を文字で入力し、音声認識の聞き間違いが原因かを切り分けます。
文字入力でも意図と違う返答になる場合は、「AI の設定」の会話用モデルと、名前・性格などの設定を確認してください。
[AIによる会話品質の違い](ai/README.md)を参照してください。
...
配布識別「2026-09-18-ui-fixes」以降の版では、確認できた導入済みモデルを候補に表示します。Ollama起動・モデル導入後に「導入済みモデルを再確認」を押してください。
