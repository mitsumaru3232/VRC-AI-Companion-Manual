...
**新規設定の初期値は、日本語・CPU用のParakeetです。** 周囲の音は「マイクと同じ方式」でモデルを共有します。
旧版でWhisper等を明示的に保存していた場合は、その設定を維持します。変更するときはこの画面で選び直してください。
...
初めて音声入力を使うときに、必要な認識モデルをインターネットから自動ダウンロードします。
初回セットアップで音声入力を有効にした場合は、初回起動時に準備します。音声入力を使わない間は取得しません。
取得済みのモデルは再利用するため、毎回ダウンロードするわけではありません。方式やモデルを変更すると、別のモデルの取得が必要になることがあります。
Parakeetでも認識モデルの初回取得は必要ですが、GPU用部品は取得しません。回線やモデルによって時間がかかります。
完了までは認識できないことがあります。空き容量と接続を確認し、必要に応じて「再接続」でやり直してください。

## WhisperでNVIDIA GPUを使う場合

必要なGPU用部品は初回に自動取得します。ダウンロードは約1.36GBです。
