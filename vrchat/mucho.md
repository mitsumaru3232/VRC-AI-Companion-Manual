...
最初の動作確認では、**VRCPetや別のKAT送信アプリを終了し、VACだけで文字盤へ送信**してください。
複数のアプリが同じ文字盤を書き換えると、文が混ざる・途中で消えるなどの競合が起きる可能性があります。

これは文字盤の確認のための手順です。VRCPetを終了した状態で、ムチォ固有の機能もすべて同じように動くとは限りません。
...
下へスクロールすると、文字盤の表示時間とひらがな変換を設定できます。

![文字盤の表示時間・ひらがな変換と、ページ下部の送信テスト。](../assets/screenshots/vrchat-bottom.png)
...
| 文が混ざる・途中で消える | VRCPetやほかのKAT送信アプリが同時に動いていないか。VACの表示時間が短すぎないか |
