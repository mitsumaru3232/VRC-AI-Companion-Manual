...
「このパソコンの AI」では、[Ollamaの導入](ollama.md)でダウンロード済みのモデルを指定してください。
配布識別「2026-09-18-ui-fixes」以降の版では、導入済みモデルだけが候補に表示されます。
「導入済みモデルを再確認」で一覧を更新できます。旧版では未導入モデルも候補に表示される場合があるため、「AI を選ぶ」で確認してください。
保存済み・手入力の名前が欄に残っていることと、モデルが現在も導入されていることは別です。
