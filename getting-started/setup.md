# 初回セットアップ

この画面では、会話用AI・名前・基本機能を順に設定します。あとから変更できます。

![初回セットアップの開始画面](../assets/screenshots/setup-1.png)

## 1. 返事を作るAIを選ぶ

「次へ」を押し、「どこの AI を使いますか」で提供元を選びます。
「使う AI モデル」には、選んだ提供元で利用可能な会話用モデルを指定します。
クラウドAIでは「利用キー」にAPIキーを貼り付けます。

![Google Geminiを選んだ例。利用キー欄に自分のAPIキーを入力します。](../assets/screenshots/setup-gemini.png)

画像のモデル名は撮影時の例です。選択肢にあるモデルでも、提供元側で提供終了・権限制限となっている場合があります。
選び方とキーの取得は、[AIの準備](../ai/README.md)を参照してください。

### このパソコンのAIを使う場合

Ollama本体と会話用モデルは、VACとは別に導入します。モデルがまだない場合は
**「セットアップをスキップ」** で進み、**「AI を選ぶ」** からモデルを導入してください。
導入後は **「アプリ → 初回セットアップを開く」** で戻れます。[Ollamaの導入手順](../ai/ollama.md)

配布識別「2026-09-18-ui-fixes」以降の版では、Ollamaから確認できた導入済みモデルだけが候補に表示されます。
Ollamaを起動・モデルを導入した後は、**「導入済みモデルを再確認」** を押してください。
モデル未導入とOllamaへの接続不可は、画面の案内に沿って確認してください。

旧版では未導入のモデルも候補に表示される場合があります。「AI を選ぶ」で導入状況を確認してください。
お使いの配布識別は、EXEと同じフォルダの「バージョン.txt」で確認できます。

## 2. 名前を決める

「AI の名前」を入力します。「あなたの名前」「AI があなたを呼ぶときの名前」は省略できます。

![名前と呼び方の設定画面](../assets/screenshots/setup-3.png)

## 3. 基本機能を選ぶ

最初はマイク・VRChat表示をオフにして、文字入力の動作確認から始めると切り分けやすくなります。
Windowsへのサインイン時の自動起動も、必要になってからオンにできます。

![マイク・VRChat表示・自動起動の選択画面](../assets/screenshots/setup-4.png)

**「設定を保存して始める」** を押します。モデルの取得が表示されたら、完了を待ってください。

## 4. 「会話」で返答を確かめる

左の「会話」を開き、下の入力欄に「こんにちは」と入力して「送信」を押します。
返答が出れば、会話用AIの準備は完了です。

「AI 未設定」や固定の確認用メッセージが出る場合は、[キーの登録とAIの切り替え](../ai/register.md)を確認してください。
文字で返答することを確かめてから、[マイク](../audio/microphone.md)と[VRChat表示](../vrchat/README.md)を設定します。

*画像は未接続の撮影環境です。キーは入力していません。*
